...
end }, { "manual",    terminal .. " -e man awesome" }, { "edit conf nvim", editor_cmd .. " " .. awesome.conffile },
...
mymainmenu = awful.menu({
    items = { { "awesome", myawesomemenu,  beautiful.awesome_icon }, { "open terminal", terminal },
        { "power options", power_options_group }, { "awesome power", awesome_power_options }, { "brave", "brave" } }
})
...
interfaces =           Bit Rate=866.7 Mb/s   Tx-Power=22 dBm   ,          Link Quality=70/70  Signal level=-33 dBm  ,          Tx excessive retries:0  Invalid misc:27   Missed beacon:0,          Mode:Managed  Frequency:5.2 GHz  Access Point: 60:7E:CD:B4:B5:38   ,          Power Management:on,          Retry short limit:7   RTS thr:off   Fragment thr:off,          Rx invalid nwid:0  Rx invalid crypt:0  Rx invalid frag:0,wlp109s0  IEEE 802.11  ESSID:"Honeydukes 5"  ,     timeout = 5
})
...
globalkeys = gears.table.join(awful.key({ modkey }, "s", hotkeys_popup.show_help, {
    ...
    end), -- awful.spawn("volume up") end),
    awful.key({}, "XF86AudioLowerVolume", function()
        awful.spawn("amixer set Master 5%-")
    end), -- awful.spawn("volume down") end),
    awful.key({}, "XF86AudioMute", function()
        -- awful.spawn("volume mute") end),
        awful.spawn("amixer -D pulse set Master 1+ toggle")
    end), --- Volume keybindings without volume MediaKeys
    ...
    awful.key({ modkey, "Ctrl", "Shift" }, "0", function()
        awful.spawn("amixer -D pulse set Master 1+ toggle")
    end), -- awful.spawn("volume mute") end),
    --- MediaKeys keybindings without MediaKeys
    awful.key({ modkey, "Ctrl", "Shift" }, "[", function()
        awful.spawn("playerctl previous")
    end), awful.key({ modkey, "Ctrl", "Shift" }, "p", function()
        awful.spawn("playerctl play-pause")
    end), awful.key({ modkey, "Ctrl", "Shift" }, "]", function()
    ...
    awful.key({ "Shift", modkey }, "#107", function()
        awful.spawn.with_shell("mkdir -p $HOME/Pictures/screenshots/ && flameshot screen -p $HOME/Pictures/screenshots/")
    end, {
...
clientkeys = gears.table.join(awful.key({ modkey }, "f", function(c)
    ...
    awful.key({ modkey }, "y", function(c)
        ...
        description = "make window sticky",
        group = "client"
    }), -- Move windows in floating layout with win+alt+arrowkeys
    awful.key({ modkey, "Mod1" }, "Down", function(c)
        c:relative_move(0, 20, 0, 0)
    end), awful.key({ modkey, "Mod1" }, "Up", function(c)
        c:relative_move(0, -20, 0, 0)
    end), awful.key({ modkey, "Mod1" }, "Left", function(c)
        c:relative_move(-20, 0, 0, 0)
    end), awful.key({ modkey, "Mod1" }, "Right", function(c)
        c:relative_move(20, 0, 0, 0)
    end), -- Re-draw windows layout in floating layout
    awful.key({ modkey, "Mod1", "Shift" }, "Up", function(c)
        c:relative_move(-20, -20, 40, 40)
    end), awful.key({ modkey, "Mod1", "Shift" }, "Down", function(c)
        c:relative_move(20, 20, -40, -40)
...
clientbuttons = gears.table.join(awful.button({}, 1, function(c)
    ...
    end), -- Custom function to make close tab with Super + Middle Click
...
for i = 1, 9 do
    globalkeys = awful.util.table.join(globalkeys, awful.key({ modkey, "Mod1" }, "#" .. i + 9, function()
        local screen = awful.screen.focused()
        local tag = screen.tags[i]
        if tag then
